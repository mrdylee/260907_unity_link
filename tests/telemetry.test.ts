...
assert.ok(snapshot.every(rack => rack.sohPercent >= 0 && rack.sohPercent <= 100));
assert.ok(snapshot.every(rack => Number.isFinite(rack.currentA)));
assert.equal(snapshot[3].state, 'IDLE');
assert.equal(snapshot[3].currentA, 0);
assert.equal(equipment.find(item => item.id === 'LCS')?.dsState, 'CLOSED');
