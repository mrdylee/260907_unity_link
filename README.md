...
- 장치 라벨에 마우스를 올리면 반투명 진단 말풍선 표시. 클릭하면 고정하며, 다른 장치를 클릭하면 전환됩니다. 닫기 버튼 또는 Esc로 닫습니다. 진단 선택은 카메라나 문을 움직이지 않습니다.
- 전체 정보: 오른쪽 진단 패널 켜기·끄기. 랙의 SOC·SOH·온도·전류·전압·STATE·진단과 LCS의 DS OPEN/CLOSE 상태, eBSC 통신 상태를 표시합니다. 주황색은 고온 경고 예제이며 모든 값은 모의 데이터입니다.
- 전체화면: 오른쪽 버튼. 종료는 Esc
